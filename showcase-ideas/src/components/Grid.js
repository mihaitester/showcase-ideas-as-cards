    ...
    for(let i = 0; i < cards.length;) {
        ...
        for(let col = 0; col < cols; col++) {
            if(i < cards.length) {
                ...
                let mh = parseInt(aspectRatio * width / cols) //height/rows > width/cols ? height/rows : width/cols
                let h = parseInt(cardSizeReducer * aspectRatio * width / cols) //height/rows > width/cols ? cardSizeReducer * height/rows : cardSizeReducer * width/cols
                row.push(<td key={i}><Card {...cards[i]} height={h} maxHeight={mh} width={cardSizeReducer * parseInt(width / cols)} maxWidth={parseInt(width / cols)}/></td>)
                i += 1;
            }
            else {
